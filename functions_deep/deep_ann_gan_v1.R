...
build_generator<-function(timesteps,nrows,nfeatures,architecture_generator,activation_generator,activation_discriminator,optimizer,dropout_generator){
  method<-'valid'
  kernel_size<-ceiling(0.25*timesteps)
  #kernel_size<-5
  #strides<-kernel_size #No overlapç
  strides<-1
  if(method=='valid')
  ...
  conv<-keras::layer_separable_conv_1d(object = input,padding = method,filters = nfeatures,strides = strides,kernel_size = kernel_size,depth_multiplier = 4,name = 'conv1d')
  model<-keras::layer_batch_normalization(object = conv)
  model<-keras::layer_activation(object = model,activation = 'relu')
  #conv<-keras::layer_conv_1d(object = input,padding = method,filters = nfeatures,strides = strides,kernel_size = kernel_size, activation = 'relu',name = 'conv1d')
...
build_discriminator<-function(timesteps,nrows,nfeatures,architecture_discriminator,activation_discriminator,optimizer){
  method<-'valid'
  kernel_size<-ceiling(0.50*timesteps)
  #kernel_size<-5
...
deep_ann_gan<-function(X=NULL,y=NULL,selected_rows=NULL,X_val=NULL,y_val=NULL,selected_rows_val=NULL,val_size=0.1,selected_columns=NULL,lr=0.001,epochs=500,kappa_validation_stop=0.8,disc_to_gen_ratio=15,batch_size=length(y),architecture_discriminator=c(50,20,10,1),architecture_generator=c(50,20,10,1),activation_discriminator='tanh',activation_generator='leaky_relu',dropout_discriminator=NULL,dropout_generator=NULL,target_ratio=NULL,view_metrics=F,view_progress_bar=F,use_gpu=F,save_model_path=NULL,log_device_placement=F,date_time_name='date_time',inpute=T,normalize=F,logfile=NULL,stateful=F,testing=F,tmpfolder=NULL,verbose=F,timesteps=15){
  ...
  for(epoch in 1:epochs){
    ...
    idxa <- sample(x = which(y_train==1),size = min(sum(y_train==1),batch_size))
    ...
    labels_alarm<-runif(n = min(length(idxa),batch_size),min = 0.7,max=1.0)
    #labels_alarm<-rep(1,length(idxa))
    labels_healthy<- runif(n = min(length(idxh),batch_size),min = 0.0,max=0.3)
    #labels_healthy<-rep(0,length(idxh))
    labels_healthy_fake<-runif(n = batch_size,min = 0.0,max=0.3)
    ...
    #sample_weight=c(rep(target_ratio,length(labels_alarm)),rep(1/target_ratio,length(labels_healthy)+length(labels_healthy_fake)))
    ...
    tmp_valid<-y_val
    k<-kappa_manual(y_real = tmp_valid,y_pred = pv,threshold=threshold)
    ...
    if(print_roc) plot(roc_obj,title=paste0('AUC: ',auc),xlim=c(1,0), ylim=c(0,1))
    
    val_met<-c(acc_manual(y_real = tmp_valid,y_pred = pv,threshold=threshold),
               precision_manual(y_real = tmp_valid,y_pred = pv,threshold=threshold),
               specificity_manual(y_real = tmp_valid,y_pred = pv,threshold=threshold),
               sensitivity_manual(y_real = tmp_valid,y_pred = pv,threshold=threshold),k)
